...
class Weights(dict):  # 管理平均感知器的权重
    ...
    def save(self, filename):
        json.dump({k: v for k, v in self._values.items() if v != 0.0},
                  open(filename, 'w'), nsure_ascii=False, indent=1)
...
class CWS:
    ...
    def update(self, x, y, delta):  # 更新权重
        for i, features in enumerate(self.gen_features(x)):
    ...
    def decode(self, x):  # 类似隐马模型的动态规划解码算法
        # 类似隐马模型中的转移概率
        # 0123 分别代表4个隐状态
        transitions = [[self.weights.get_value(str(i)+':'+str(j), 0) for j in range(4)] for i in range(4)]
        # 类似隐马模型中的发射概率
        emissions = [[sum(self.weights.get_value(str(tag)+feature, 0) for       feature in features) for tag in range(4)] for features in self.gen_features(x)]
        print(transitions)
        # 类似隐马模型中的前向概率
        alphas = [[[e, None] for e in emissions[0]]]
        for i in range(len(x)-1):
            alphas.append([max([alphas[i][j][0]+transitions[j][k]+emissions[i+1][k], j] for j in range(4)) for k in range(4)])
...
def load_example(words: list) -> tuple:  # 词数组，得到x，y
    """
    words：词语列表
    return:
    """
    y = []    # 样本标签，每个字在词语中的位置 0123，对应BMES
    for word in words:
        if len(word) == 1:
            y.append(3)
        else:
...
if __name__ == '__main__':
    ...
    test = map(lambda x: x.split('/')[0], file.read().split(' '))
    file.close()
    # print(list(test))
    cws = CWS()
    # 迭代5次
